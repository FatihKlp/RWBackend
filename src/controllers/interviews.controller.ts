...
export const getInterviewById = async (req: Request, res: Response) => {
    ...
    try {
        const interview = await Interview.findById(id).populate('user questionPackage');  // Kullanıcı ve soru paketleriyle birlikte getir
        if (!interview) {
...
export const updateInterview = async (req: Request, res: Response) => {
    ...
    try {
        const updatedInterview = await Interview.findByIdAndUpdate(id, updateData, { new: true }).populate('user questionPackage'); // Kullanıcı ve soru paketleriyle birlikte getir
        if (!updatedInterview) {
            return res.status(404).json({ message: 'Mülakat bulunamadı' });
        }
