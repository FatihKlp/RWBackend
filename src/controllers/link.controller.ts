...
export const accessInterviewByLink = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (!interview.publish) {
      res.status(403).json({ message: "Bu mülakat yayınlanmamıştır." });
      return;
    }
    ...
    if (new Date(interview.expireDate) < currentDate) {
      res.status(403).json({ message: "Bu mülakatın süresi dolmuştur." });
      return;
    }

    res.status(200).json(interview); // Mülakat soruları ile dönüyor
    return;
  } catch (error) {
